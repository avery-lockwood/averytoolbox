# -*- coding: utf-8 -*-
"""Inventor-like design, sketch, thread, assembly, and drawing workflow for FreeCAD 1.1+.
...
def _sketch_region_faces(sketch):
    """Split the sketch plane by all sketch curves and return the enclosed faces.

    This is the planar-arrangement step Onshape runs implicitly: every curve is
    cut at every intersection and the fragments are assembled into one face per
    enclosed region, so regions bounded by *portions* of edges work too.
    Faces are returned in the sketch's parent (body-local) coordinates.
    """
    shape = getattr(sketch, "Shape", None)
    edges = list(getattr(shape, "Edges", []) or [])
    if not edges:
        return []
    compound = Part.makeCompound([edge.copy() for edge in edges])
    local = compound.copy()
    local.transformShape(sketch.Placement.inverse().toMatrix())
    box = local.BoundBox
    margin = max(box.DiagonalLength * 0.25, 10.0)
    plane = Part.makePlane(
        box.XLength + 2.0 * margin,
        box.YLength + 2.0 * margin,
        App.Vector(box.XMin - margin, box.YMin - margin, 0.0),
    )
    plane.Placement = sketch.Placement.multiply(plane.Placement)
    border = plane.OuterWire
    from BOPTools import SplitAPI

    # Fuse the curves against each other first so they are split at their
    # mutual intersections; without this, crossing curves (e.g. two
    # overlapping circles) imprint as whole wires and the lens-shaped
    # in-between areas never become separate faces.
    cutter = compound
    if len(edges) > 1:
        try:
            cutter = edges[0].copy().fuse([edge.copy() for edge in edges[1:]])
        except Exception:
            cutter = compound
    sliced = SplitAPI.slice(plane, [cutter], "Split", 1e-6)
    faces = []
    for face in sliced.Faces:
        # Faces touching the artificial outer border are the unbounded
        # leftovers, not sketch regions.
        try:
            if face.distToShape(border)[0] < 1e-6:
                continue
        except Exception:
            pass
        faces.append(face)
...
class SketchRegionProxy(object):
    """Parametric 'clicked region' of a sketch, identified by seed points.

    Identity is resolved by point-in-region on every recompute (the same way
    Onshape resolves region selections), so the feature follows sketch edits
    for as long as each seed point still lands inside an enclosed area.
    """

    def __init__(self, obj):
        if "Source" not in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyLink", "Source", "Region", "Sketch providing the curves"
            )
        if "SeedPoints" not in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyVectorList",
                "SeedPoints",
                "Region",
                "Sketch-local points identifying the picked regions",
            )
        obj.Proxy = self

    def execute(self, obj):
        sketch = obj.Source
        seeds = list(obj.SeedPoints or [])
        if sketch is None or not seeds:
            obj.Shape = Part.Shape()
            return
        faces = _sketch_region_faces(sketch)
        chosen = []
        used = set()
        missing = 0
        for seed in seeds:
            point = sketch.Placement.multVec(seed)
            found = False
            for position, face in enumerate(faces):
                if position in used:
                    continue
                try:
                    inside = face.isInside(point, 1e-5, True)
                except Exception:
                    inside = False
                if inside:
                    chosen.append(face)
        ...
        if not chosen:
            raise ValueError(
                "No enclosed sketch region contains the stored seed point(s); "
                "re-pick the regions after this sketch edit."
            )
        if missing:
            App.Console.PrintWarning(
                "[Inventor-like UI] {}: {} seed point(s) no longer fall inside a "
                "region and were skipped.\n".format(obj.Label, missing)
            )
        obj.Shape = chosen[0] if len(chosen) == 1 else Part.makeCompound(chosen)
...
class NativeThreadCatalog(object):
    ...
    def __init__(self, document):
        ...
        self.probe.Label = "Temporary Inventor-like thread catalog"
        try:
...
class HoleThreadDialog(QtWidgets.QDialog):
    """Inventor-style front end over FreeCAD's native threaded Hole feature."""
    ...
    def __init__(self, hole, is_new=False, parent=None):
        ...
        self.setWindowTitle("Hole — Inventor-style Threaded Hole")
        self.resize(560, 820)
...
class StandaloneThreadDialog(QtWidgets.QDialog):
    """Inventor-like independent Thread dialog for selected cylindrical faces."""
    ...
    def __init__(self, catalog, analyses, existing=None, parent=None):
        ...
        self.setWindowTitle("Thread — Inventor-style Independent Feature")
        self.resize(500, 560)
        outer = QtWidgets.QVBoxLayout(self)
    ...
    def attach(self, obj):
        if "FusionThreadVersion" not in obj.PropertiesList:
            obj.addProperty(
                ...
                "FusionThreadVersion",
                "Thread",
                "Inventor-like thread feature schema version",
            )
...
class FusionHoleCommand(object):
    def GetResources(self):
        return {
            ...
            "ToolTip": "Create or edit a Inventor-style threaded Hole using native FreeCAD geometry",
            "Accel": "H",
        }
...
class CommandPalette(QtWidgets.QDialog):
    ...
    def __init__(self, profile):
        ...
        self.search.setPlaceholderText("Search commands (S)")
...
class FusionProfile(QtCore.QObject):
    ...
    WORKSPACES = (
        ...
        ("CAM", ("CAMWorkbench", "PathWorkbench")),
        ("STUDIO", ("RenderWorkbench", "RaytracingWorkbench")),
    )

    # Onshape-style keyboard map. Keys apply contextually: SKETCH_KEYS while a
    # sketch is in edit, MODEL_KEYS otherwise. Each entry is a tuple of command
    # ids tried in order; inactive commands are skipped.
    SKETCH_KEYS = {
        QtCore.Qt.Key_L: ("Sketcher_CreateLine",),
        QtCore.Qt.Key_R: ("Sketcher_CreateRectangle",),
        QtCore.Qt.Key_F: ("Sketcher_CreateRectangle_Center",),
        QtCore.Qt.Key_C: ("Sketcher_CreateCircle",),
        QtCore.Qt.Key_A: ("Sketcher_Create3PointArc",),
        QtCore.Qt.Key_O: ("Sketcher_Offset",),
        QtCore.Qt.Key_X: ("Sketcher_Trimming",),
        QtCore.Qt.Key_Q: ("Sketcher_ToggleConstruction",),
        QtCore.Qt.Key_D: ("Sketcher_Dimension", "Sketcher_ConstrainDistance"),
        QtCore.Qt.Key_T: ("Sketcher_ConstrainTangent",),
        QtCore.Qt.Key_H: ("Sketcher_ConstrainHorizontal", "Sketcher_ConstrainHorVer"),
        QtCore.Qt.Key_V: ("Sketcher_ConstrainVertical", "Sketcher_ConstrainHorVer"),
        QtCore.Qt.Key_I: ("Sketcher_ConstrainCoincidentUnified", "Sketcher_ConstrainCoincident"),
        QtCore.Qt.Key_B: ("Sketcher_ConstrainParallel",),
        QtCore.Qt.Key_Equal: ("Sketcher_ConstrainEqual",),
        QtCore.Qt.Key_M: ("Sketcher_Translate",),
    }

    SKETCH_SHIFT_KEYS = {
        QtCore.Qt.Key_L: ("Sketcher_ConstrainPerpendicular",),
        QtCore.Qt.Key_O: ("Sketcher_ConstrainCoincidentUnified", "Sketcher_ConstrainCoincident"),
        QtCore.Qt.Key_Q: ("Sketcher_ConstrainSymmetric",),
        QtCore.Qt.Key_M: ("Sketcher_ConstrainSymmetric",),
        QtCore.Qt.Key_J: ("Sketcher_ConstrainBlock",),
        QtCore.Qt.Key_S: ("Sketcher_CreatePoint",),
        QtCore.Qt.Key_F: ("Sketcher_CreateFillet",),
        QtCore.Qt.Key_A: ("Sketcher_CreatePolyline",),
    }

    MODEL_KEYS = {
        QtCore.Qt.Key_M: ("Std_Measure",),
        QtCore.Qt.Key_I: ("Std_Measure",),
        QtCore.Qt.Key_F: ("Std_ViewFitAll",),
        QtCore.Qt.Key_W: ("Std_ViewBoxZoom",),
        QtCore.Qt.Key_Z: ("Std_ViewZoomOut",),
        QtCore.Qt.Key_N: ("Std_AlignToSelection",),
        QtCore.Qt.Key_V: ("Std_ToggleVisibility",),
    }

    MODEL_SHIFT_KEYS = {
        QtCore.Qt.Key_S: ("PartDesign_NewSketch",),
        QtCore.Qt.Key_F: ("PartDesign_Fillet",),
        QtCore.Qt.Key_Z: ("Std_ViewZoomIn",),
        QtCore.Qt.Key_T: ("Std_ToggleTransparency",),
        QtCore.Qt.Key_V: ("Std_HideSelection",),
        QtCore.Qt.Key_B: ("Std_ShowSelection",),
    }

    VIEW_SHIFT_KEYS = {
        QtCore.Qt.Key_1: ("Std_ViewFront",),
        QtCore.Qt.Key_2: ("Std_ViewRear",),
        QtCore.Qt.Key_3: ("Std_ViewLeft",),
        QtCore.Qt.Key_4: ("Std_ViewRight",),
        QtCore.Qt.Key_5: ("Std_ViewTop",),
        QtCore.Qt.Key_6: ("Std_ViewBottom",),
        QtCore.Qt.Key_7: ("Std_ViewIsometric",),
    }
    ...
    def apply(self):
        ...
        # Reopen the last Inventor-like workspace when it still exists. Part
    ...
    def _finish_apply(self):
        try:
            ...
            self.mw.statusBar().showMessage(
                "Inventor-like profile active — Sketch mode is contextual; Drawing uses Insert Model View; Assembly Ctrl+C/V duplicates instances",
                7000,
            )
        except Exception:
    ...
    def restore(self):
        ...
        if self.task_dock is not None:
            try:
                self.task_dock.setFloating(False)
                self.task_dock.show()
            except Exception:
                pass
            self.task_dock = None
    ...
    def _arrange_docks(self):
        ...
        # The standalone task panel becomes a small floating overlay that only
        # appears while a task dialog is active (see _refresh_task_overlay).
        self.task_dock = task if combo is None else None
        if self.task_dock is not None:
            self.task_dock.hide()

    def _task_panel_in_use(self):
        """True when the Tasks pane has anything to show — an active task
        dialog, or visible watcher/content boxes (e.g. Sketcher's panels)."""
        try:
            ...
            pass
        dock = self.task_dock
        inner = dock.widget() if dock is not None else None
        if inner is None:
            return False
        for child in inner.findChildren(QtWidgets.QWidget):
            try:
                class_name = str(child.metaObject().className())
            except Exception:
                continue
            if (
                "ActionGroup" in class_name
                or "TaskBox" in class_name
                or "TaskGroup" in class_name
                or isinstance(child, QtWidgets.QGroupBox)
            ) and child.isVisibleTo(inner):
                return True
        return False

    def _refresh_task_overlay(self):
        """Show the Tasks pane as a compact overlay only while it has content."""
        dock = self.task_dock
        if dock is None:
            return
        dialog_active = self._task_panel_in_use()
        if dialog_active and not dock.isVisible():
            dock.setFloating(True)
            dock.resize(330, 480)
            anchor = self.mw.centralWidget() or self.mw
            try:
                dock.move(anchor.mapToGlobal(QtCore.QPoint(14, 14)))
            except Exception:
                pass
            dock.show()
        elif not dialog_active and dock.isVisible() and dock.isFloating():
            dock.hide()
    ...
    def _remove_runtime_widgets(self):
        ...
        self._clear_section_clip(silent=True)
        if self.region_pick is not None:
            self.finish_region_pick(cancelled=True)
        self.joint_diagnostic_timer.stop()
        self.joint_diagnostic_panel = None
        self.joint_diagnostic_label = None
    ...
    def _add_callback_dropdown(self, toolbar, label, rows, icon_command=None):
        """Add a Inventor-style dropdown whose rows call Python workflow helpers."""
        button = QtWidgets.QToolButton(toolbar)
        button.setObjectName("{}Dropdown_{}".format(TOOLBAR_PREFIX, re.sub(r"\W+", "", label)))
        button.setText(label)
        button.setToolButtonStyle(QtCore.Qt.ToolButtonTextUnderIcon)
    ...
    def _build_quick_toolbar(self):
        quick = self._make_toolbar(
            "FusionLike_Quick", "Inventor-like: Workspace", QtCore.Qt.ToolButtonTextBesideIcon
        )
        ...
        self.workspace_combo.setToolTip("Inventor-style workspace selector")
    ...
    def _build_design_toolbars(self):
        create = self._make_toolbar("FusionLike_Create", "Inventor-like: Create")
        ...
        self._add_callback_dropdown(
            ...
            (
                ...
                (
                    "Join / Pad picked regions (Shift+R)",
                    lambda: self.start_region_pick("PartDesign_Pad"),
                    "FusionLike_ExtrudeRegionAdd",
                    "Click inside sketch loops (even between crossing curves), Enter to Pad them",
                ),
                (
                    "Cut / Pocket picked regions",
                    lambda: self.start_region_pick("PartDesign_Pocket"),
                    "FusionLike_ExtrudeRegionCut",
                    "Click inside sketch loops (even between crossing curves), Enter to Pocket them",
                ),
            ),
            "PartDesign_Pad",
        )
        self._add_dropdown(
            ...
            "Hole",
            (
                ("Hole & Thread (Inventor-style)", HOLE_COMMAND_ID),
        ...
        self._add_callback_dropdown(
            ...
            (
                (
                    "Join / Revolution",
                    lambda: self.start_extrude("PartDesign_Revolution"),
                    "FusionLike_RevolveAdd",
                    "Finish the active sketch when needed and create a Revolution",
                ),
                (
                    "Cut / Groove",
                    lambda: self.start_extrude("PartDesign_Groove"),
                    "FusionLike_RevolveCut",
                    "Finish the active sketch when needed and create a Groove",
                ),
                (
                    "Join / Revolution picked regions",
                    lambda: self.start_region_pick("PartDesign_Revolution"),
                    "FusionLike_RevolveRegionAdd",
                    "Click inside sketch loops, Enter to revolve them (dialog asks for the axis)",
                ),
                (
                    "Cut / Groove picked regions",
                    lambda: self.start_region_pick("PartDesign_Groove"),
                    "FusionLike_RevolveRegionCut",
                    "Click inside sketch loops, Enter to groove them (dialog asks for the axis)",
                ),
            ),
        ...
        modify = self._make_toolbar("FusionLike_Modify", "Inventor-like: Modify")
        ...
        inspect = self._make_toolbar("FusionLike_Inspect", "Inventor-like: Construct and Inspect")
        self._section_label(inspect, "WORK FEATURES / INSPECT")
        for command_id in (
            "Part_DatumPlane",
            "Part_DatumLine",
            "Part_DatumPoint",
        ...
        self._add_callback_command(
            inspect,
            "Section View",
            self.toggle_face_section,
            "FusionLike_FaceSection",
            "Section along a selected face/plane with a drag + rotate manipulator (Shift+X); "
            "with nothing selected, opens the capped Persistent Section Cut",
            "Part_SectionCut",
        )
    ...
    def _build_sketch_toolbars(self):
        finish = self._make_toolbar(
            "FusionLike_SketchFinish", "Inventor-like: Finish Sketch", QtCore.Qt.ToolButtonTextBesideIcon
        )
        self._section_label(finish, "EXIT")
        for command_id in (
        ...
        create = self._make_toolbar("FusionLike_SketchCreate", "Inventor-like: Sketch Create")
        self._section_label(create, "CREATE")
        for command_ids in (
        ...
        modify = self._make_toolbar("FusionLike_SketchModify", "Inventor-like: Sketch Modify")
        self._section_label(modify, "MODIFY")
        for command_ids in (
        ...
        constrain = self._make_toolbar("FusionLike_SketchConstrain", "Inventor-like: Sketch Constrain")
        ...
        self._add_callback_command(
            constrain,
            "fx Parameter",
            self.parameterize_selected_dimension,
            "FusionLike_AddParameter",
            "Bind the selected (or newest) dimension to a named document parameter (Shift+D)",
            "Sketcher_Dimension",
        )

        inspect = self._make_toolbar("FusionLike_SketchInspect", "Inventor-like: Sketch Inspect")
    ...
    def _build_assembly_toolbars(self):
        ...
        assemble = self._make_toolbar("FusionLike_Assembly", "Inventor-like: Assemble")
        ...
        joints = self._make_toolbar("FusionLike_AssemblyJoints", "Inventor-like: Assembly Joints")
        ...
        motion = self._make_toolbar("FusionLike_AssemblyMotion", "Inventor-like: Motion and Output")
    ...
    def _build_drawing_toolbars(self):
        create = self._make_toolbar("FusionLike_DrawingCreate", "Inventor-like: Drawing Create")
        self._section_label(create, "CREATE")
        self._add_dropdown(
            create,
            "New Sheet",
        ...
        modify = self._make_toolbar("FusionLike_DrawingModify", "Inventor-like: Drawing Modify")
        ...
        dims = self._make_toolbar("FusionLike_DrawingDimensions", "Inventor-like: Drawing Dimensions")
        ...
        annotate = self._make_toolbar("FusionLike_DrawingAnnotate", "Inventor-like: Drawing Annotate")
        ...
        self._add_callback_command(
            ...
            "Insert or refresh a standards-based callout from a native Hole or Inventor-like Thread",
            "TechDraw_RichTextAnnotation",
        )

        output = self._make_toolbar("FusionLike_DrawingOutput", "Inventor-like: Drawing Output")
    ...
    def show_drawing_workflow_help(self):
        QtWidgets.QMessageBox.information(
            self.mw,
            "Inventor-like Drawing Workflow",
    ...
    def paste_assembly_components(self, in_place=False):
        ...
        if not rows:
            QtWidgets.QMessageBox.warning(
                ...
                "The Inventor-like assembly clipboard is empty. Select component instances and use Copy first.",
            )
            return False
    ...
    def insert_thread_callout(self):
        ...
        if source is None:
            QtWidgets.QMessageBox.warning(
                self.mw,
                "Thread Callout",
                "Select a threaded Part Design Hole, a Inventor-like Thread feature, or an existing thread-callout annotation.",
            )
            return False
    ...
    def _build_timeline(self):
        dock = QtWidgets.QDockWidget("FEATURE BROWSER (TIMELINE)", self.mw)
        ...
        timeline.setIconSize(QtCore.QSize(16, 16))
        ...
        container.setMaximumHeight(52)
        dock.setWidget(container)
        self.mw.addDockWidget(QtCore.Qt.BottomDockWidgetArea, dock)
        dock.show()
        try:
            self.mw.resizeDocks([dock], [52], QtCore.Qt.Vertical)
        except Exception:
            pass
    ...
    def _refresh_timeline_if_needed(self):
        ...
        # Inventor-like groups after the new workbench has registered its actions.
        current = _active_workbench_name()
    ...
    # ---------- Inventor-like Hole and independent Thread workflows ----------
    ...
    def open_fusion_hole(self):
        ...
        transaction_name = "Create Inventor-style threaded Hole" if is_new else "Edit threaded Hole"
    ...
    def open_fusion_thread(self):
        ...
        context["document"].openTransaction(
            "Edit Inventor-like Thread" if existing is not None else "Create Inventor-like Thread"
        )
        try:
            feature = existing
            if feature is None:
    ...
    # ---------- Inventor-like persistent projection workflow ----------
    ...
    def _projection_warning(self, text):
        ...
        QtWidgets.QMessageBox.warning(self.mw, "Inventor-like projection", text)
    ...
    def _projection_requirements(self):
        if _freecad_version_tuple() < (1, 1, 0):
            self._projection_warning(
                ...
                "Upgrade FreeCAD, then run Inventor-like → Apply / rebuild."
            )
            return None
        sketch = self._active_edited_sketch()
        if sketch is None:
    ...
    def _set_binder_metadata(self, binder, key, source, sub_name, sketch):
        ...
        for name, description in definitions:
            if name not in properties:
                binder.addProperty("App::PropertyString", name, "Inventor-like Projection", description)
    ...
    def _selected_sketch_region(self):
        """Return (sketch, edge names) when sketch edges are selected."""
        try:
        ...
        for sel in selection:
            obj = sel.Object
            if obj is None or not _is_derived(obj, "Sketcher::SketchObject"):
                continue
            edges = [
                _element_leaf(name)
                for name in sel.SubElementNames
                if _element_leaf(name).startswith("Edge")
            ]
            if edges:
                return obj, edges
        return None, []

    def _extrude_region_binder(self, sketch, edges, command_id):
        """Onshape-style partial extrude: bind selected sketch edges into a
        SubShapeBinder and run Pad/Pocket on it instead of the whole sketch."""
        doc = sketch.Document
        try:
            if _active_edited_sketch_global() is not None:
                Gui.ActiveDocument.resetEdit()
                doc.recompute()
        except Exception:
            pass
        try:
            doc.openTransaction("Extrude sketch region")
            binder = doc.addObject("PartDesign::SubShapeBinder", "RegionBinder")
            binder.Label = "Region_{}".format(sketch.Label)
            body = _global_body_for_object(sketch) or _active_partdesign_body(doc)
            if body is not None:
                body.addObject(binder)
            binder.Support = [(sketch, tuple(edges))]
            doc.recompute()
            closed = any(w.isClosed() for w in getattr(binder.Shape, "Wires", []) or [])
            if not closed:
                doc.abortTransaction()
                QtWidgets.QMessageBox.information(
                    self.mw,
                    "Extrude region",
                    "The selected edges do not form a closed loop. Select all edges "
                    "bounding the region(s) you want to extrude, then try again.",
                )
                return False
            doc.commitTransaction()
        except Exception:
            try:
                ...
                pass
            _console(
                "Region binder failed:\n{}".format(traceback.format_exc()), warning=True
            )
            return False
        try:
            if hasattr(binder.ViewObject, "Visibility"):
            ...
            pass
        self.mw.statusBar().showMessage(
            "Extruding region binder '{}' — it stays linked to {}".format(
                binder.Label, sketch.Label
            ),
            6000,
        )
    ...
    def start_extrude(self, command_id):
        region_sketch, region_edges = self._selected_sketch_region()
        if region_sketch is not None:
            return self._extrude_region_binder(region_sketch, region_edges, command_id)
        sketch = self._active_edited_sketch()
        if sketch is None:
            return self._run_command(command_id)
    ...
    # ---------- Onshape-style region picking (click inside a loop) ----------

    def _resolve_region_sketch(self):
        """Sketch to pick regions from: selection, else the active body's
        newest visible sketch, else the document's newest visible sketch."""
        try:
            selection = Gui.Selection.getSelectionEx("", 0)
        except Exception:
            selection = []
        for sel in selection:
            if sel.Object is not None and _is_derived(sel.Object, "Sketcher::SketchObject"):
                return sel.Object
        doc = App.ActiveDocument
        if doc is None:
            return None
        body = _active_partdesign_body(doc)
        pools = []
        if body is not None:
            pools.append([
                obj for obj in getattr(body, "Group", []) or []
                if _is_derived(obj, "Sketcher::SketchObject")
            ])
        pools.append([
            obj for obj in doc.Objects if _is_derived(obj, "Sketcher::SketchObject")
        ])
        for candidates in pools:
            ...
            visible = [
                ...
                if bool(getattr(getattr(obj, "ViewObject", None), "Visibility", False))
            ]
            return (visible or candidates)[-1]
        return None

    def _region_highlight_setup(self, view):
        try:
            from pivy import coin
        except Exception:
            return None
        try:
            ...
            material.diffuseColor.setValue(1.0, 0.55, 0.1)
            material.emissiveColor.setValue(0.4, 0.2, 0.0)
    ...
    def _region_highlight_add(self, state, face):
        root = state.get("highlight")
        if root is None:
            return
        try:
            from pivy import coin
            ...
            try:
                ...
                global_placement = sketch.Placement
            shown = face.copy()
            shown.transformShape(
                global_placement.multiply(sketch.Placement.inverse()).toMatrix()
            )
            # Nudge the overlay off the sketch plane to avoid z-fighting.
            normal = global_placement.Rotation.multVec(App.Vector(0, 0, 1))
            offset = normal.multiply(0.08)
            vertices, triangles = shown.tessellate(0.3)
            coordinates = coin.SoCoordinate3()
            coordinates.point.setValues(
                0,
                len(vertices),
                [[v.x + offset.x, v.y + offset.y, v.z + offset.z] for v in vertices],
            )
            face_set = coin.SoIndexedFaceSet()
            index = []
            for triangle in triangles:
                index.extend([triangle[0], triangle[1], triangle[2], -1])
            face_set.coordIndex.setValues(0, len(index), index)
            piece = coin.SoSeparator()
            piece.addChild(coordinates)
            piece.addChild(face_set)
            root.addChild(piece)
        except Exception:
            pass

    def _region_highlight_clear(self, state):
        root = state.get("highlight")
        if root is None:
            return
        try:
            state["view"].getSceneGraph().removeChild(root)
        except Exception:
            pass
        state["highlight"] = None

    def start_region_pick(self, command_id=None):
        """Click inside sketch loops (even between crossing curves) to select
        regions; Enter extrudes them, Esc cancels. E or Shift+R."""
        if self.region_pick is not None:
            return self.finish_region_pick()
        if self._selected_sketch_region()[0] is not None:
            # Boundary edges are already selected — use the binder flow.
            return bool(self.show_extrude_menu())
        sketch = _active_edited_sketch_global()
        if sketch is not None:
            try:
                Gui.ActiveDocument.resetEdit()
                sketch.Document.recompute()
            ...
            sketch = self._resolve_region_sketch()
        if sketch is None:
            # Nothing to pick from — fall back to the classic whole-sketch flow.
            return bool(self.show_extrude_menu())
        try:
            faces = _sketch_region_faces(sketch)
        except Exception:
            _console(
                "Region arrangement failed:\n{}".format(traceback.format_exc()),
                warning=True,
            )
            faces = []
        if not faces:
            QtWidgets.QMessageBox.information(
                self.mw,
                "Pick sketch regions",
                "No enclosed areas were found in '{}'. Regions must be fully "
                "bounded by sketch curves.".format(sketch.Label),
            )
            return False
        gui_document = getattr(Gui, "ActiveDocument", None)
        ...
        state = {
            ...
            "view": view,
            "command": command_id,
            "highlight": self._region_highlight_setup(view),
        }
        state["callback"] = view.addEventCallback(
            "SoMouseButtonEvent", self._region_pick_click
        )
        self.region_pick = state
        Gui.Selection.clearSelection()
        self.mw.statusBar().showMessage(
            "Pick regions of '{}': click inside each area to extrude — Enter to "
            "finish, Esc to cancel".format(sketch.Label),
            0,
        )
        return True

    def _region_pick_click(self, info):
        state = self.region_pick
        if state is None:
            return
        ...
        try:
            ...
            view = state["view"]
            near_point = view.getPoint(int(position[0]), int(position[1]))
            ...
            try:
                ...
                global_placement = sketch.Placement
            base = global_placement.Base
            normal = global_placement.Rotation.multVec(App.Vector(0, 0, 1))
            denominator = direction.dot(normal)
            if abs(denominator) < 1e-9:
                self.mw.statusBar().showMessage(
                    "The view is parallel to the sketch plane — rotate a little and click again",
                    4000,
                )
                return
            scale = (base - near_point).dot(normal) / denominator
            clicked = near_point + direction * scale
            seed_local = global_placement.inverse().multVec(clicked)
            test_point = sketch.Placement.multVec(seed_local)
            hit = None
            for face in state["faces"]:
                try:
                    if face.isInside(test_point, 1e-5, True):
                        hit = face
                        break
                except Exception:
                    continue
            if hit is None:
                self.mw.statusBar().showMessage(
                    "No enclosed region there — click inside a bounded area "
                    "({} picked so far)".format(len(state["seeds"])),
                    4000,
                )
                return
            state["seeds"].append(seed_local)
            self._region_highlight_add(state, hit)
            self.mw.statusBar().showMessage(
                "{} region(s) picked — Enter to extrude, Esc to cancel".format(
                    len(state["seeds"])
                ),
                0,
            )
    ...
    def finish_region_pick(self, cancelled=False):
        state = self.region_pick
        self.region_pick = None
        if state is None:
            return False
        try:
            state["view"].removeEventCallback("SoMouseButtonEvent", state["callback"])
        except Exception:
            pass
        self._region_highlight_clear(state)
        self.mw.statusBar().clearMessage()
        if cancelled or not state["seeds"]:
            self.mw.statusBar().showMessage("Region picking ended", 3000)
            return True
        sketch = state["sketch"]
        doc = sketch.Document
        try:
            doc.openTransaction("Pick sketch regions")
            region = doc.addObject("Part::Part2DObjectPython", "SketchRegion")
            SketchRegionProxy(region)
            region.ViewObject.Proxy = 0
            region.Label = "Region_{}".format(sketch.Label)
            body = _global_body_for_object(sketch) or _active_partdesign_body(doc)
            if body is not None:
                body.addObject(region)
            region.Source = sketch
            region.SeedPoints = state["seeds"]
            doc.recompute()
            if not getattr(region.Shape, "Faces", None):
                doc.abortTransaction()
                QtWidgets.QMessageBox.warning(
                    self.mw,
                    "Pick sketch regions",
                    "Could not build faces for the picked regions.",
                )
                return False
            doc.commitTransaction()
        except Exception:
            try:
                ...
                pass
            _console(
                "Region feature failed:\n{}".format(traceback.format_exc()),
                warning=True,
            )
            return False
        try:
            Gui.Selection.clearSelection()
            Gui.Selection.addSelection(region)
        except Exception:
            pass
        ...
        if command_id:
            ...
            QtCore.QTimer.singleShot(180, self.show_extrude_menu)
        return True

    # ---------- Onshape-style section view from a face or plane ----------

    def _selected_section_placement(self):
        """Placement of the first selected planar face or datum/origin plane."""
        try:
            selection = Gui.Selection.getSelectionEx("", 0)
        except Exception:
            selection = []
        for sel in selection:
            obj = sel.Object
            sub_objects = list(getattr(sel, "SubObjects", []) or [])
            sub_names = list(getattr(sel, "SubElementNames", []) or [])
            for position, shape in enumerate(sub_objects):
                face = shape if hasattr(shape, "Surface") else None
                if face is None or not hasattr(face, "normalAt"):
                    continue
                surface = face.Surface
                if surface.__class__.__name__ != "Plane":
                    continue
                sub_name = sub_names[position] if position < len(sub_names) else ""
                base = face.CenterOfMass
                normal = face.normalAt(0, 0)
                # SubObjects are already in global coordinates for the root
                # selection object, so use them directly.
                rotation = App.Rotation(App.Vector(0, 0, 1), normal)
                return App.Placement(base, rotation), "{}.{}".format(obj.Label, _element_leaf(sub_name))
            if obj is not None and str(getattr(obj, "TypeId", "")) in (
                "App::Plane",
                "PartDesign::Plane",
                "Part::DatumPlane",
            ):
                ...
                return placement, str(obj.Label)
        return None, ""

    def _clear_section_clip(self, silent=False):
        view = self.section_clip_view
        self.section_clip_view = None
        if view is None:
            return False
        try:
            ...
            return False
        if not silent:
            self.mw.statusBar().showMessage("Section view off", 4000)
        return True

    def toggle_face_section(self):
        """Shift+X: section the model along a picked face/plane with a
        draggable, rotatable manipulator. No planar selection falls back to
        the native capped Persistent Section Cut dialog."""
        if self.section_clip_view is not None:
            return self._clear_section_clip()
        placement, source = self._selected_section_placement()
        if placement is None:
            return self._run_first_active(("Part_SectionCut", "Std_SectionCut"))
        gui_document = getattr(Gui, "ActiveDocument", None)
        view = getattr(gui_document, "ActiveView", None) if gui_document else None
        if view is None or not hasattr(view, "toggleClippingPlane"):
            return self._run_first_active(("Part_SectionCut", "Std_SectionCut"))
        try:
            Gui.Selection.clearSelection()
            # noManip=False attaches the drag/rotate manipulator.
            view.toggleClippingPlane(1, False, False, placement)
        except Exception:
            _console(
                "Face section failed:\n{}".format(traceback.format_exc()), warning=True
            )
            return self._run_first_active(("Part_SectionCut", "Std_SectionCut"))
        self.section_clip_view = view
        self.mw.statusBar().showMessage(
            "Section view along {} — drag the plane to slide it, drag the ball to tilt it; Shift+X again to exit".format(
                source or "selection"
            ),
            8000,
        )
    ...
    # ---------- Inventor-style fx parameters ----------

    PARAMETER_SET_NAME = "Parameters"
    PARAMETER_DATUM_TYPES = ("Distance", "DistanceX", "DistanceY", "Radius", "Diameter", "Angle")

    def _parameter_container(self, doc):
        """Return (creating on demand) the document's shared parameter store.

        Prefers an App::VarSet (FreeCAD 1.1) and falls back to a spreadsheet.
        """
        existing = doc.getObject(self.PARAMETER_SET_NAME)
        if existing is not None and existing.TypeId in ("App::VarSet", "Spreadsheet::Sheet"):
            return existing
        for candidate in doc.Objects:
            if candidate.TypeId in ("App::VarSet", "Spreadsheet::Sheet") and str(
                getattr(candidate, "Label", "")
            ) == "Parameters":
                return candidate
        try:
            container = doc.addObject("App::VarSet", self.PARAMETER_SET_NAME)
        except Exception:
            container = doc.addObject("Spreadsheet::Sheet", self.PARAMETER_SET_NAME)
        container.Label = "Parameters"
        return container

    def _store_parameter(self, container, name, value, is_angle):
        """Write value into the container under name and return an expression."""
        if container.TypeId == "App::VarSet":
            if name not in container.PropertiesList:
                property_type = "App::PropertyAngle" if is_angle else "App::PropertyLength"
                container.addProperty(property_type, name, "Parameters", "Inventor-style user parameter")
            setattr(container, name, value)
            return "{}.{}".format(container.Name, name)

        # Spreadsheet fallback: name in column A, aliased value in column B.
        row = None
        for candidate in range(1, 1000):
            try:
                alias = container.getAlias("B{}".format(candidate))
            except Exception:
                alias = None
            if alias == name:
                row = candidate
                break
            if not alias and not str(container.getContents("A{}".format(candidate)) or ""):
                row = candidate
                break
        if row is None:
            return None
        container.set("A{}".format(row), name)
        container.set("B{}".format(row), "{}{}".format(value, " deg" if is_angle else " mm"))
        try:
            container.setAlias("B{}".format(row), name)
        except Exception:
            pass
        return "{}.{}".format(container.Name, name)

    def _selected_datum_constraint(self, sketch):
        """Selected driving datum constraint index, else the newest one."""
        indices = []
        try:
            selection = Gui.Selection.getSelectionEx("", 0)
        except Exception:
            selection = []
        for sel in selection:
            if getattr(sel.Object, "Name", None) != sketch.Name:
                continue
            for sub_name in sel.SubElementNames:
                if sub_name.startswith("Constraint"):
                    try:
                        indices.append(int(sub_name[len("Constraint"):]) - 1)
                    except ValueError:
                        continue
        ...
        selected = [
            i
            for i in indices
            if 0 <= i < len(constraints)
            and constraints[i].Type in self.PARAMETER_DATUM_TYPES
            and getattr(constraints[i], "Driving", True)
        ]
        if selected:
            return selected[-1]
        for i in range(len(constraints) - 1, -1, -1):
            constraint = constraints[i]
            if constraint.Type in self.PARAMETER_DATUM_TYPES and getattr(constraint, "Driving", True):
                return i
        return None

    def parameterize_selected_dimension(self):
        """Promote a sketch dimension to a named document parameter (Shift+D)."""
        sketch = _active_edited_sketch_global()
        if sketch is None:
            QtWidgets.QMessageBox.information(
                self.mw,
                "fx Parameter",
                "Open a sketch and select (or just create) a dimension first.",
            )
            return False
        index = self._selected_datum_constraint(sketch)
        if index is None:
            QtWidgets.QMessageBox.information(
                self.mw,
                "fx Parameter",
                "This sketch has no driving dimension to parameterize.",
            )
            return False
        constraint = sketch.Constraints[index]
        is_angle = constraint.Type == "Angle"
        if is_angle:
            value = math.degrees(constraint.Value)
            shown = "{:.4g} deg".format(value)
        else:
            value = constraint.Value
            shown = "{:.4g} mm".format(value)
        default_name = str(constraint.Name or "") or "d{}".format(index)
        name, accepted = QtWidgets.QInputDialog.getText(
            self.mw,
            "fx Parameter",
            "Parameter name for {} = {}:".format(constraint.Type, shown),
        ...
        doc = sketch.Document
        try:
            doc.openTransaction("fx Parameter")
            container = self._parameter_container(doc)
            expression = self._store_parameter(container, name, value, is_angle)
            if not expression:
                doc.abortTransaction()
                QtWidgets.QMessageBox.warning(
                    self.mw, "fx Parameter", "Could not store the parameter value."
                )
                return False
            try:
                if str(constraint.Name or "") != name:
                    sketch.renameConstraint(index, name)
            except Exception:
                pass
            constraint_name = str(sketch.Constraints[index].Name or "")
            if constraint_name:
                path = "Constraints.{}".format(constraint_name)
            else:
                path = "Constraints[{}]".format(index)
            ...
            try:
                ...
                pass
            _console("fx Parameter failed:\n{}".format(traceback.format_exc()), warning=True)
            QtWidgets.QMessageBox.warning(self.mw, "fx Parameter", str(exc))
            return False
        self.mw.statusBar().showMessage(
            "Dimension bound to {} = {} — edit it on the '{}' object".format(
                expression, shown, container.Label
            ),
            7000,
        )
        return True
    ...
    def show_extrude_menu(self):
        """One menu for every profile outcome: pad, pocket, revolve, groove.

        Revolving a region profile asks for an axis in the dialog (regions do
        not carry the sketch's own axes) — a straight construction edge in the
        sketch makes a handy pick.
        """
        menu = QtWidgets.QMenu("Extrude / Revolve", self.mw)
        rows = (
            ("Join / Pad", "PartDesign_Pad"),
            ("Cut / Pocket", "PartDesign_Pocket"),
            ("Join / Revolution", "PartDesign_Revolution"),
            ("Cut / Groove", "PartDesign_Groove"),
        )
        actions = []
        for label, command_id in rows:
            source = self._find_action(command_id)
            action = QAction(source.icon() if source else QIcon(), label, menu)
            menu.addAction(action)
            actions.append((action, command_id))
        chosen = _menu_exec(menu, QCursor.pos())
        for action, command_id in actions:
            if chosen == action:
                return bool(self.start_extrude(command_id))
        return True
    ...
    def eventFilter(self, watched, event):
        ...
        in_sketch = _active_edited_sketch_global() is not None
        if self.region_pick is not None:
            if key in (QtCore.Qt.Key_Return, QtCore.Qt.Key_Enter):
                return bool(self.finish_region_pick())
            if key == QtCore.Qt.Key_Escape:
                return bool(self.finish_region_pick(cancelled=True))
            return False
        if key == QtCore.Qt.Key_Escape and self.projection_observer is not None:
            self.end_projection_mode()
            return True

        # Onshape view keys (Shift+1..7) work in every context.
        if modifiers == QtCore.Qt.ShiftModifier and key in self.VIEW_SHIFT_KEYS:
            return bool(self._run_first_active(self.VIEW_SHIFT_KEYS[key]))
        ...
        # Independent Thread moved off Shift+T (now transparency, Onshape-style).
        if key == QtCore.Qt.Key_T and modifiers == (
            QtCore.Qt.ControlModifier | QtCore.Qt.ShiftModifier
        ):
            return bool(self.open_fusion_thread())

        if modifiers == QtCore.Qt.ShiftModifier and key == QtCore.Qt.Key_R:
            return bool(self.start_region_pick())
        if modifiers == QtCore.Qt.ShiftModifier:
            if in_sketch:
                if key == QtCore.Qt.Key_D:
                    return bool(self.parameterize_selected_dimension())
                if key == QtCore.Qt.Key_P:
                    return bool(self.project_reference())
                if key in self.SKETCH_SHIFT_KEYS:
                    return bool(self._run_first_active(self.SKETCH_SHIFT_KEYS[key]))
                return False
            ...
            if key == QtCore.Qt.Key_X:
                return bool(self.toggle_face_section())
            if key in self.MODEL_SHIFT_KEYS:
                return bool(self._run_first_active(self.MODEL_SHIFT_KEYS[key]))
            return False
        ...
        if in_sketch:
            if key == QtCore.Qt.Key_U:
                return bool(self.project_profile())
            if key == QtCore.Qt.Key_E:
                # Whole-sketch extrude: finish the sketch, then Join/Cut menu.
                return bool(self.show_extrude_menu())
            if key in self.SKETCH_KEYS:
                return bool(self._run_first_active(self.SKETCH_KEYS[key]))
            return False
        if key == QtCore.Qt.Key_H:
            return bool(self.show_hole_menu())
        if key == QtCore.Qt.Key_E:
            # Region picking: click inside loops, Enter -> Join/Cut menu.
            return bool(self.start_region_pick())
        if key == QtCore.Qt.Key_Q:
            return bool(self.show_press_pull_menu())
        if assembly_context and key == QtCore.Qt.Key_I:
            return bool(self._run_first_active(("Assembly_InsertLink",)))
        if key in self.MODEL_KEYS:
            return bool(self._run_first_active(self.MODEL_KEYS[key]))
        return False
...
def install_menu():
    ...
    menu = QtWidgets.QMenu("Inventor-like", menu_bar)
    menu.setObjectName(MENU_NAME)

    apply_action = menu.addAction("Apply / rebuild Inventor-like profile")
    search_action = menu.addAction("Command Search (S)")
    timeline_action = menu.addAction("Show / hide feature browser (timeline)")
    menu.addSeparator()
    hole_action = menu.addAction("Hole & Thread (H)")
    thread_action = menu.addAction("Independent Thread (Ctrl+Shift+T)")
    parameter_action = menu.addAction("fx: Parameterize dimension (Shift+D in sketch)")
    section_action = menu.addAction("Section view from face / plane (Shift+X)")
    region_action = menu.addAction("Pick sketch regions to extrude (Shift+R)")

    projection_menu = menu.addMenu("Project Geometry")
    ...
    parameter_action.triggered.connect(parameterize_dimension)
    section_action.triggered.connect(toggle_face_section)
    region_action.triggered.connect(pick_sketch_regions)
...
def parameterize_dimension():
    global _PROFILE
    if _PROFILE is None:
        _PROFILE = FusionProfile()
    return _PROFILE.parameterize_selected_dimension()


def toggle_face_section():
    global _PROFILE
    if _PROFILE is None:
        _PROFILE = FusionProfile()
    return _PROFILE.toggle_face_section()


def pick_sketch_regions():
    global _PROFILE
    if _PROFILE is None:
        _PROFILE = FusionProfile()
    return _PROFILE.start_region_pick()
